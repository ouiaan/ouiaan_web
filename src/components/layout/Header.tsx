...
export function Header() {
  ...
  const NavLink = ({ href, label, className }: { href: string; label: string, className?: string }) => (
    <Link href={href} passHref>
      <span
        className={cn(
          'font-headline uppercase tracking-wider text-xl transition-colors duration-300',
          pathname === href
            ? 'text-accent'
        ...
        <div className="absolute top-24 left-0 w-full bg-background/95 backdrop-blur-sm md:hidden flex flex-col items-center gap-6 py-8 z-50">
            ...
            <DropdownMenu>
                <DropdownMenuTrigger className={cn(
                    'group flex items-center gap-1 font-headline uppercase tracking-wider text-2xl transition-colors duration-300 outline-none',
                    pathname.startsWith('/store')
                    ? 'text-accent'
                    : 'text-foreground/70 hover:text-foreground'
                )}>
                ...
                <DropdownMenuContent>
                    <DropdownMenuItem asChild>
                        <Link href="/store/photo" onClick={() => setMobileMenuOpen(false)}>Photo</Link>
                    </DropdownMenuItem>
                    <DropdownMenuItem asChild>
                        <Link href="/store/video" onClick={() => setMobileMenuOpen(false)}>Video</Link>
                    </DropdownMenuItem>
                </DropdownMenuContent>
            </DropdownMenu>
        </div>
